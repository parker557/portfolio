...
const renderTasks = async (user) => {
	console.log("aaa:", user.id)
	const tasks = await axios.get('/api/tasks/users/' + user.id)
	...
	for (let task of tasksArray) {
		...
		taskHeading.addEventListener('click', () => {
			renderTaskDetails(task, tasksArray, user)
		})
