...
const renderEditTask = (task, tasksArray, user) => {
  ...
  document.getElementById('edit-task-form').addEventListener('submit', (event) => {
    handleSubmitForm(event, task, tasksArray, user)
  })
}

const handleSubmitForm = (event, task, tasksArray, user) => {
  ...
  return axios
    ...
    .then(async (res) => {
      async function renderNew() {
        ...
        // debugger
        renderTaskDetails(currentTask, response.data, user)
      }
      await renderNew()
